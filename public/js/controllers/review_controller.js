function ReviewController($scope, $http) {
	...
	function getMovieReviews(imdbID) {
		$http.get(`${server}/movies/${imdbID}/reviews`).then(function(response) {
	...
	function addReview(currentUser, movie) {
		$http
			...
			.then(function(response) {
				$http.post(`${server}/movies/${response.data.movie.id}/reviews`, {
					review: {
						title: self.newReview.title,
						rating: self.newReview.rating,
						comments: self.newReview.comments,
	...
	function editReview(review, currentUser) {
		$http
			.put(`${server}/reviews/` + review.id, {
				review: {
					title: self.updatedReview.title,
					rating: self.updatedReview.rating,
					comments: self.updatedReview.comments
				}
			})
			.then(function(response) {
				self.updatedReview = "";
	...
	function deleteReview(review, currentUser) {
		$http.delete(`${server}/reviews/` + review.id).then(function(response) {
			getAllReviews();
		});
	}
